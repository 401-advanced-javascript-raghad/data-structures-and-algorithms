...
describe('fifo-animal-shelter.js', () => {
  it('can create a new empty shelter', () => {
    ...
    expect(emptyShelter).toBeTruthy();
    expect(emptyShelter.front).toBeNull();
    expect(emptyShelter.back).toBeNull();
  });
  ...
  it('will return an error message if anything other than dog or cat is enqueued', () => {
    const shelter = new AnimalShelter();
  ...
  it('will dequeue the animal  this.front if no preference is given', () => {
    const shelter = new AnimalShelter();
    let animal1 = 'cat';
    let animal2 = 'cat';
    let animal3 = 'dog';
  ...
  it('will return a useful message if preferred animal is not in the shelter', () => {
    ...
    expect(shelter.dequeue('cat')).toBe('no cat inside the shelter');
  });
});